...
def add_movies_to_list(movie_ids, bluray):
    ...
    COVER_BASE_URL = 'http://www.einthusan.tv/images/covers/'
    if (bluray):
        BASE_URL = 'http://www.einthusan.tv/movies/watch.php?bluray=true&id='
    else:
        BASE_URL = 'http://www.einthusan.tv/movies/watch.php?id='
...
def list_music_videos(name, url, language, mode):
    if (url == "" or url == None):
        url = 'http://www.einthusan.tv/music/index.php?lang=' + language 
...
def preferred_server(lnk, mainurl):
	...
	if location != 'No Preference':
		...
		for i in servers:
			urltry = ("https://" + CDN_PREFIX + str(i+SERVER_OFFSET[0]) + ".einthusan.tv/" + vidpath)
